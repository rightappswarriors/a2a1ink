...
class Consumers extends CI_Controller {
    ...
    public function remove(){
        ...
        // Get consumer info to delete photo file
        $consumer = $this->consumers_model->view_info($id)->row();
        ...
        $datalog = array(
            'user' => $this->session->userdata('arallink_userid'),
            'description' => $this->session->userdata('arallink_username')." has deleted a consumer with ID: ".$id.".",
            'dateadded' => date("Y-m-d H:i:s")
        );
        $this->historylog_model->insert($datalog);
