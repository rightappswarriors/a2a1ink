...
<header class="header contain-to-grid">
	<?php
	// Load org info for logo
	$CI =& get_instance();
	$CI->load->model('dashboard_model');
	$accountid = $CI->session->userdata('arallink_accountid');
	$org_logo_url = '';
	$default_logo_url = base_url() . 'arallink.png';
	if ($accountid) {
	...
	<div class="mobile-menu-toggle">
		...
		<a href="<?= site_url() ?>" class="mobile-logo">
			...
			<?php else: ?>
				<img src="<?= $default_logo_url ?>" alt="Default Logo" class="org-logo-img">
			<?php endif; ?>
			<b>AralLink</b> RFID
		</a>
	</div>
	...
	<aside class="sidebar" id="sidebar">
		...
		<nav class="sidebar-nav">
			<ul class="left">
				...
				<li class="item <?= ($active_menu == 'settings' ? 'active' : '') ?>">
